...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen pt-24 pb-16 px-4 md:px-8 max-w-7xl mx-auto aurora grain space-y-8">
      {/* Welcome Header */}
      ...
      <div className="glass rounded-2xl border border-emerald-500/30 bg-surface/40 p-6 md:p-7 relative overflow-hidden shadow-lg">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
          <div className="space-y-2 max-w-xl">
            ...
            <h3 className="font-display font-bold text-xl text-fg">
              Earn <span className="text-gradient">₹100</span> for every friend who buys a project
            </h3>
            <p className="text-xs md:text-sm text-muted leading-relaxed">
              Share your link with classmates. When they sign up and purchase any final-year project, ₹100 is credited to your wallet for UPI withdrawal.
            </p>
            {referralLink ? (
              <div className="flex flex-wrap items-center gap-2 pt-1">
                <div className="flex items-center gap-2 bg-bg/80 border border-line rounded-xl px-3 py-1.5 max-w-full">
                  <span className="text-xs font-mono font-bold text-violet-txt select-all truncate">
                    {referralLink}
                  </span>
                  <button
          ...
          <div className="flex flex-col sm:flex-row lg:flex-col items-start lg:items-end gap-3 shrink-0 border-t lg:border-t-0 border-line/60 pt-4 lg:pt-0">
            <div className="grid grid-cols-3 gap-3 w-full sm:w-auto text-center">
              <div className="rounded-xl border border-line bg-surface-hi/40 p-2.5 min-w-20">
                <span className="block font-bold text-emerald-400 text-base">
                  ₹{balanceData?.availableBalance ?? 0}
                </span>
                <span className="text-[10px] text-muted">Withdrawable</span>
              </div>
              <div className="rounded-xl border border-line bg-surface-hi/40 p-2.5 min-w-20">
                <span className="block font-bold text-fg text-base">
                  ₹{balanceData?.totalEarned ?? 0}
                </span>
                <span className="text-[10px] text-muted">Total Earned</span>
              </div>
              <div className="rounded-xl border border-line bg-surface-hi/40 p-2.5 min-w-20">
                <span className="block font-bold text-cyan text-base">
                  {referredUsersData?.users.length ?? 0}
                </span>
                <span className="text-[10px] text-muted">Friends Joined</span>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-bg/80 backdrop-blur-sm">
          <div className="glass rounded-2xl border border-line p-6 max-w-md w-full space-y-5 relative shadow-2xl">
            ...
            <form onSubmit={handleChangePassword} className="space-y-4">
              <Field label="Current Password" hint="Leave blank if you previously signed up with Google">
